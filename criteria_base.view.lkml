...
explore: criteria_joins_base {
  ...
  join: ad_group {
    ...
    sql_on: ${criteria.ad_group_id} = ${ad_group.ad_group_id} AND
      ...
      ${criteria.date_date} = ${ad_group.date_date} ;;
  ...
  join: campaign {
    ...
    sql_on: ${criteria.campaign_id} = ${campaign.campaign_id} AND
      ...
      ${criteria.date_date} = ${campaign.date_date};;
    relationship: many_to_one
  }
  join: customer {
    view_label: "Customer"
    sql_on: ${criteria.external_customer_id} = ${customer.external_customer_id} AND
      ${criteria.date_date} = ${customer.date_date} ;;
    relationship: many_to_one
  }
}
...
view: criteria_base {
  extension: required
  extends: [date_base, date_primary_key_base, google_adwords_base]
