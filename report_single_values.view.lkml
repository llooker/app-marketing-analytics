view: report_single_values {
  derived_table: {
    sql:
      ...
      nonconverting_ads_and_keywords as (
          WITH ad_group_avg_cpa AS (
            SELECT
              ad_impressions.ExternalCustomerId  AS external_customer_id,
              ad_impressions.CampaignId  AS campaign_id,
      ...
      UNION ALL
      select 'keyword_and_ad_alerts' as key, count(*) as value, null as compare from keyword_and_ad_alerts
      UNION ALL
      select 'nonconverting_ads_and_keywords' as key, count(*) as value, null as compare from nonconverting_ads_and_keywords
    ;;
  }
  dimension: key {
    type:  string
