    ...
    sql:
      WITH underperforming_ad_groups as (
        ...
        LEFT JOIN adwords_v201609.AdGroup_6747157124  AS ad_group ON ad_impressions.AdGroupId = ad_group.AdGroupId AND
              (CAST((TIMESTAMP(ad_impressions._DATA_DATE))  AS DATE)) = (CAST((TIMESTAMP(ad_group._DATA_DATE))  AS DATE))
      ...
      underperforming_keywords AS (
        ...
        WHERE (((REPLACE(keyword.Status, "Status_", "")) = 'Active')) AND (((((TIMESTAMP(ad_group._DATA_DATE)) ) >= ((TIMESTAMP_TRUNC(CAST(TIMESTAMP_TRUNC(CURRENT_TIMESTAMP(), DAY) AS TIMESTAMP), QUARTER))) AND ((TIMESTAMP(ad_group._DATA_DATE)) ) < ((TIMESTAMP(CONCAT(CAST(DATE_ADD(CAST(TIMESTAMP_TRUNC(CAST(TIMESTAMP_TRUNC(CAST(TIMESTAMP_TRUNC(CURRENT_TIMESTAMP(), DAY) AS TIMESTAMP), QUARTER) AS TIMESTAMP), QUARTER) AS DATE), INTERVAL 3 MONTH) AS STRING), ' ', CAST(TIME(CAST(TIMESTAMP_TRUNC(CAST(TIMESTAMP_TRUNC(CAST(TIMESTAMP_TRUNC(CURRENT_TIMESTAMP(), DAY) AS TIMESTAMP), QUARTER) AS TIMESTAMP), QUARTER) AS TIMESTAMP)) AS STRING))))))))
      ...
      underperforming_ads AS (
        ...
        WHERE (((REPLACE(ad.Status, "Status_", "")) = 'Enabled')) AND (((((TIMESTAMP(ad_group._DATA_DATE)) ) >= ((TIMESTAMP_TRUNC(CAST(TIMESTAMP_TRUNC(CURRENT_TIMESTAMP(), DAY) AS TIMESTAMP), QUARTER))) AND ((TIMESTAMP(ad_group._DATA_DATE)) ) < ((TIMESTAMP(CONCAT(CAST(DATE_ADD(CAST(TIMESTAMP_TRUNC(CAST(TIMESTAMP_TRUNC(CAST(TIMESTAMP_TRUNC(CURRENT_TIMESTAMP(), DAY) AS TIMESTAMP), QUARTER) AS TIMESTAMP), QUARTER) AS DATE), INTERVAL 3 MONTH) AS STRING), ' ', CAST(TIME(CAST(TIMESTAMP_TRUNC(CAST(TIMESTAMP_TRUNC(CAST(TIMESTAMP_TRUNC(CURRENT_TIMESTAMP(), DAY) AS TIMESTAMP), QUARTER) AS TIMESTAMP), QUARTER) AS TIMESTAMP)) AS STRING))))))))
      ...
      nonconverting_ads as (
          ...
          LEFT JOIN ad_group_avg_cpa ON ad_impressions.ExternalCustomerId = ad_group_avg_cpa.external_customer_id AND
                ...
                ad_impressions.AdGroupId = ad_group_avg_cpa.ad_group_id
          WHERE
            ((REPLACE(ad.Status, "Status_", "")) = 'Enabled')
          GROUP BY 1,2,3,4,5,6,7,8
      ...
      nonconverting_keywords as (
        WITH ad_group_avg_cpa AS (
          SELECT
            ...
            COALESCE(SUM(ad_impressions.Impressions ), 0) AS impressions,
            COALESCE(SUM(ad_impressions.Interactions ), 0) AS interactions
          FROM adwords_v201609.AdGroupBasicStats_6747157124
          AS ad_impressions
          GROUP BY 1,2,3
        )
          SELECT
          ...
          LEFT JOIN ad_group_avg_cpa ON ad_impressions.ExternalCustomerId = ad_group_avg_cpa.external_customer_id AND
                ...
                ad_impressions.AdGroupId = ad_group_avg_cpa.ad_group_id
          WHERE
            ((REPLACE(keyword.Status, "Status_", "")) = 'Active')
          GROUP BY 1,2,3,4,5,6
          HAVING (COALESCE(SUM(ad_impressions.Conversions ), 0) = 0) AND ((COALESCE(SUM((ad_impressions.Cost / 1000000) ), 0)) / NULLIF(((COALESCE(SUM(ad_group_avg_cpa.cost_usd ), 0)) / NULLIF((COALESCE(SUM(ad_group_avg_cpa.conversions ), 0)),0)),0)  > 4)
      )

      select 'underperforming_ad_groups' as key, count(*) as value, null as compare from underperforming_ad_groups
      UNION ALL
      select 'underperforming_keywords' as key, count(*) as value, null as compare from underperforming_keywords
      UNION ALL
      select 'underperforming_ads' as key, count(*) as value, null as compare from underperforming_ads
      UNION ALL
      select 'keyword_alerts' as key, count(*) as value, null as compare from keyword_alerts
      UNION ALL
      select 'nonconverting_ads' as key, count(*) as value, null as compare from nonconverting_ads
      UNION ALL
      select 'ad_alerts' as key, count(*) as value, null as compare from ad_alerts
      UNION ALL
      select 'nonconverting_keywords' as key, count(*) as value, null as compare from nonconverting_keywords
    ;;
  }
  dimension: key {
    type:  string
